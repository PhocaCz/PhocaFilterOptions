...
document.addEventListener("DOMContentLoaded", function () {
	...
	function filterOptions(str) {
		...
		var tabContent = '#configContent div.tab-pane, #config-document div.tab-pane, .tab-content .tab-pane';
		...
		if (str.length > 0) {
			...
			document.querySelectorAll(tab).forEach(function (elem) {
				/* J4 elem.classList.add('disabled'),
    ...
    elem.classList.remove('show')*/
		...
		document.querySelectorAll(tab).forEach(function (elem) {
			/* J4 elem.classList.remove('disabled'),
   elem.classList.remove('active'),
   elem.classList.remove('show')*/
			elem.classList.remove('active');
		});
